...
function getAgGrid() {
  ...
  if (!AG)
    throw new Error(
      "AG Grid UMD não carregado. Verifique a ORDEM dos scripts e o path do CDN.",
    );
  return AG;
}
...
function currencyFormatter(p) {
  ...
  let n =
    typeof p.value === "number"
      ? p.value
      : parseCurrencyFlexible(p.value, currency);
...
function frontToNumberFirst(s) {
  ...
  const str = String(s);
  const m =
    str.match(/-?\d{1,3}(?:\.\d{3})*,\d{2}/) || str.match(/-?\d+(?:\.\d+)?/);
  if (!m) return null;
...
function blurLikeClickElsewhere(p) {
  ...
  const allCols = (api.getAllDisplayedColumns?.() || []).map((c) =>
    c.getColId(),
  );
  const neighborColId =
    allCols.find((id) => id !== colId && id !== "campaign_status") || colId;
...
EditableMoneyCellRenderer.prototype.updateVisibility = function () {
  ...
  const isEditable =
    typeof editableProp === "function" ? !!editableProp(p) : !!editableProp;
  ...
  this.ok.style.display =
    showBase && !hasError && justSaved ? "inline-flex" : "none";
...
const LionStatusMenu = (() => {
  ...
  function open({ left, top, width, current, pick, anchor = null }) {
    ...
    ["ACTIVE", "PAUSED"].forEach((st) => {
      ...
      item.className =
        "lion-status-menu__item" + (current === st ? " is-active" : "");
      item.textContent = st;
...
StatusSliderRenderer.prototype.init = function (p) {
  ...
  const interactive = new Set(
    Array.isArray(cfg.interactiveLevels) ? cfg.interactiveLevels : [0],
  );
  ...
  const getVal = () =>
    String(
      p.data?.campaign_status ?? p.data?.status ?? p.value ?? "",
    ).toUpperCase();
  ...
  const computeTrackLen = () => {
    ...
    const edgeGap = parseFloat(
      getComputedStyle(root).getPropertyValue("--edge-gap") || "0",
    );
  ...
  root.addEventListener(
    "touchstart",
    (e) => beginDrag(e.touches[0].clientX, e),
    { passive: false },
  );
...
StatusSliderRenderer.prototype.refresh = function (p) {
  ...
  const kRectW =
    this.eGui.querySelector(".ag-status-knob").getBoundingClientRect().width ||
    0;
  ...
  requestAnimationFrame(() => {
    ...
    const onNudge =
      parseFloat(cs.getPropertyValue("--knob-on-nudge") || "0") || 0;
    const offNudge =
      parseFloat(cs.getPropertyValue("--knob-off-nudge") || "0") || 0;
...
StackBelowRenderer.prototype.init = function (p) {
  ...
  const onlyLevel0 = !!p?.colDef?.cellRendererParams?.onlyLevel0;
  ...
  const fmt = String(p?.colDef?.cellRendererParams?.format || "raw");
  const formatVal = (v) => {
    if (v == null) return "";
  ...
  topEl.textContent =
    formatVal(typeof topVal === "number" ? topVal : cc_numberOr0(topVal)) ||
    stripHtml(topVal ?? "");
  ...
  parts.forEach((row) => {
    ...
    const val = Number.isFinite(row?.value)
      ? row.value
      : cc_numberOr0(row?.value);
...
async function updateAdStatusBackend(id, status) {
  ...
  if (!res.ok || data?.ok === false)
    throw new Error(data?.error || "Ad status update failed");
...
async function updateAdsetStatusBackend(id, status) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data?.ok === false)
    throw new Error(data?.error || "Adset status update failed");
  await withMinSpinner(t0, MIN_SPINNER_MS);
  return data;
}
...
async function updateCampaignBidTypeBackend(id, bidType) {
  ...
  const res = await fetch(
    `/api/campaigns/${encodeURIComponent(id)}/bid_type/`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      credentials: "same-origin",
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data?.ok === false)
    throw new Error(data?.error || "Bid Type update failed");
  await withMinSpinner(t0, MIN_SPINNER_MS);
  return data;
}
...
async function updateCampaignBidBackend(id, bidNumber) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data?.ok === false)
    throw new Error(data?.error || "Bid update failed");
  await withMinSpinner(t0, MIN_SPINNER_MS);
  return data;
}
...
(function CalcColsPopulate() {
  ...
  function getSelectableColumns(api) {
    ...
    let items = displayed
      .map((col) => {
        const def = col.getColDef?.() || col.colDef || {};
        return {
          field: def.field || def.colId || String(col.getColId?.() || ""),
          headerName:
            def.headerName ||
            def.header ||
            def.field ||
    ...
    items = items.filter((it) =>
      seen.has(it.field) ? false : seen.add(it.field),
    );
  ...
  // Renderiza <option> em um <select>, preservando a seleção anterior se ainda existir
  function fillSelect(selectEl, items, keepValue) {
    if (!selectEl) return;
    const placeholder =
      selectEl.getAttribute("data-kt-select-placeholder") || "Select";

    // Fix: Placeholder não deve ser disabled/selected quando temos keepValue
    const placeholderAttrs =
      keepValue && items.some((it) => it.field === keepValue)
        ? 'value=""'
        : 'value="" disabled selected';

    const opts = [
      "<option " + placeholderAttrs + ">" + placeholder + "</option>",
    ].concat(
      items.map(
        (it) =>
          `<option value="${escapeHtml(it.field)}">${escapeHtml(it.headerName)} (${escapeHtml(
            it.field,
          )})</option>`,
      ),
    );
    ...
    if (keepValue && items.some((it) => it.field === keepValue)) {
      selectEl.value = keepValue;
    }

    // Re-hidratar o componente KT select se sua lib expõe um refresh
    try {
      // Algumas variantes: ajuste para sua infra (qualquer uma que existir será chamada)
      selectEl.dispatchEvent(
        new CustomEvent("kt:select:refresh", { bubbles: true }),
      );
...
const LionCompositeColumns = (() => {
  ...
  function _getColumnDefs(api) {
    ...
    if (typeof api.getColumnDefs === "function")
  ...
  function activate(ids = []) {
    ...
    if (!group || !Array.isArray(group.children)) {
      console.warn(
        '[CompositeColumns] Grupo "Metrics & Revenue" não encontrado',
      );
      return;
    }
    ...
    ids.forEach((id) => {
      ...
      const exists = newChildren.some(
        (c) => String(c.colId || c.field) === colKey,
      );
      if (exists) return;

      // usa meta __after (ou headerName/field) para posicionar; fallback = 'Revenue'
      const afterKey =
        (colDef.__after && String(colDef.__after).trim()) || "Revenue";
  ...
  function deactivate(ids = []) {
    ...
    const newChildren = group.children.filter(
      (c) => !idsSet.has(String(c.colId || c.field)),
    );
...
const LionCalcColumns = (() => {
  ...
  function _buildColDef(cfg) {
    ...
    const tooltipValueGetter = (p) => {
      const row = p?.data || {};
      const tot =
        totalFn && Number.isFinite(totalFn(row)) ? totalFn(row) : null;
  ...
  function openQuickBuilder() {
    const headerName = prompt(
      "Header da coluna (ex.: EPC Custom):",
      "EPC Custom",
    );
    ...
    const format = (
      prompt("Formato (currency|int|raw):", "currency") || "currency"
    ).toLowerCase();
...
BidTypeFloatingFilter.prototype.init = function (params) {
  ...
  sel.className =
    "ag-input-field-input ag-text-field-input lion-ff-select--inputlike";
  sel.style.width = "100%";
  ...
  const opts = [["", "ALL"]].concat(
    (Array.isArray(BID_TYPE_VALUES)
      ? BID_TYPE_VALUES
      : ["LOWEST_COST", "COST_CAP"]
    ).map((code) => [code, BID_TYPE_LABEL?.[code] || code]),
  );
...
CampaignStatusFloatingFilter.prototype.onParentModelChanged = function (
  parentModel,
) {
  if (!this.sel) return;
  if (!parentModel) {
...
AccountStatusFloatingFilter.prototype.onParentModelChanged = function (
  parentModel,
) {
  if (!this.sel) return;
  if (!parentModel) {
      ...
      },
      {
        ...
        onCellValueChanged: async (p) => {
          try {
            ...
            if (Number.isFinite(oldN) && Number.isFinite(newN) && oldN === newN)
              return;
...
function getSelectionColId(api) {
  try {
    ...
    const found = cols.find(
      (c) =>
        c.getColDef()?.headerCheckboxSelection ||
        c.getColDef()?.checkboxSelection,
    );
...
function togglePinnedColsFromCheckbox(silent = false) {
  ...
  if (!silent)
    showToast(
      checked ? "Columns Pinned" : "Columns Unpinned",
...
(function setupToolbar() {
  ...
  function listPresetNames() {
    return Object.keys(readPresets()).sort((a, b) =>
  ...
  (function setupCalcColsModal() {
    ...
    if (btn) {
      btn.addEventListener("click", (e) => {
        ...
        const sel =
          btn.getAttribute("data-kt-modal-toggle") || "#calcColsModal";
    ...
    function readForm() {
      ...
      const format = ($("#cc-format")?.value || "currency")
        .trim()
        .toLowerCase();
  ...
  (function initSizeModeToggle() {
    ...
    el.addEventListener("change", () => {
      ...
      applySizeMode(next);
      showToast(
        next === "auto" ? "Mode: Auto Size" : "Mode: Size To Fit",
        "info",
      );
    });
    ...
    if (activePreset) {
      ...
      if (api) {
        try {
          ...
          console.log(
            "[Preset] Complete initialization applied - no active preset",
          );
        } catch (e) {
...
function makeGrid() {
  ...
  const autoGroupColumnDef = {
    ...
    pinned: "left",
    cellClass: (p) =>
      ["camp-root", "camp-child", "camp-grand"][
        Math.min(p?.node?.level ?? 0, 2)
      ],
    cellClassRules: {
    ...
    try {
      const col =
        ...
        api
          ...
          .find((c) => c.getColId?.() === "campaign");
      if (!col) return 300;
      const comp = api.getDisplayedColAfter
        ? api.getDisplayedColAfter(col)
        : null; // só p/ forçar layout
  ...
  const gridOptions = {
    ...
    isServerSideGroup: (data) =>
      data?.__nodeType === "campaign" || data?.__nodeType === "adset",
      ...
      const dataSource = {
        getRows: async (req) => {
          try {
            ...
            const filterModelWithGlobal =
              buildFilterModelWithGlobal(filterModel);
            ...
            if (groupKeys.length === 0) {
              ...
              try {
                ...
                targetApi?.setPinnedBottomRowData?.([pinnedTotal]) ||
                  targetApi?.setGridOption?.("pinnedBottomRowData", [
                    pinnedTotal,
                  ]);
              } catch (e) {
            ...
            if (groupKeys.length === 2) {
              ...
              const data = await fetchJSON(
                `${DRILL_ENDPOINTS.ADS}?${qs.toString()}`,
              );
